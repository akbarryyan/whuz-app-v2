        ...
        <div className="flex flex-wrap items-center gap-2">
          {paymentMethods.map((pm) => (
            pm.img ? (
              ...
              <span
                key={pm.name}
                className="bg-slate-200 text-slate-700 rounded-lg px-3 py-1.5 text-xs font-bold tracking-wide"
              >
                {pm.name}
              </span>
            )
            ...
            {socialLinks.map((s) =>
              SOCIAL_ICONS[s.platform] ? (
                <a key={s.platform} href={s.href}
                  className="text-[#003D99] hover:text-purple-700 transition-colors">
                  {SOCIAL_ICONS[s.platform]}
                </a>
              ) : null
            )}
