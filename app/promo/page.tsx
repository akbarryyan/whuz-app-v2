    ...
    <div className={`${quicksand.className} flex min-h-screen justify-center bg-[#F5F5F5]`}>
      <div className="relative w-full max-w-[480px] min-h-screen bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto pt-[60px] pb-24">
          ...
          {/* ── Hero banner ── */}
          <div
            className="w-full px-5 py-4 relative overflow-hidden flex items-center justify-between"
          ...
          {/* ── Search ── */}
          <div className="px-4 py-3 bg-white sticky top-[60px] z-10 border-b border-slate-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-sm bg-slate-100 rounded-xl border-none outline-none text-slate-700 placeholder-slate-400"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
          ...
          <div className="px-4 py-4">
            ...
            {/* Section title */}
            <p className="text-sm font-bold text-[#003D99] mb-4">
                ...
                {filtered.map((promo) => (
                  ...
                  >
                    ...
                    <div className="px-4 py-3">
                      <p className="text-[11px] font-bold text-[#003D99] leading-snug">{promo.title}</p>
                      {promo.description && (
                        <p className="text-[10px] text-slate-500 mt-0.5 line-clamp-2">{promo.description}</p>
                      )}
                      {promo.endDate && (
                        <p className="text-[11px] text-slate-400 mt-1.5">
                          Berlaku s.d{" "}
                          <span className="font-semibold text-slate-500">{formatDate(promo.endDate)}</span>
                        </p>
                      )}
                    </div>
                  </div>
